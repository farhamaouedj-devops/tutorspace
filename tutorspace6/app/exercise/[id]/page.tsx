    ...
    <div className="grid-bg flex flex-col" style={{ height: '100vh', overflow: 'hidden' }}>
      ...
      <header className="flex items-center justify-between px-4 py-2.5 flex-shrink-0"
        ...
        <div className="flex items-center gap-3">
          <Link href="/dashboard" className="w-8 h-8 flex items-center justify-center rounded-lg text-sm"
            style={{ background: 'rgba(86,96,112,0.2)', color: '#8a9ab0' }}>←</Link>
          <div className="flex items-center gap-2">
            {subject && (
              <span className="text-xs px-2 py-0.5 rounded-full hidden sm:inline"
                    style={{ background: `${(subject as any).color}20`, color: (subject as any).color, border: `1px solid ${(subject as any).color}40` }}>
                {subject.icon} {subject.name}
              </span>
            )}
            <h1 className="text-sm font-semibold" style={{ fontFamily: 'Syne', color: '#faf7f0' }}>{exercise.title}</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="px-3 py-1.5 rounded-full text-xs font-semibold"
               style={{ background: isAdmin ? 'rgba(48,96,192,0.2)' : 'rgba(232,160,48,0.15)', color: isAdmin ? '#93c5fd' : '#fbbf24', border: `1px solid ${isAdmin ? 'rgba(48,96,192,0.4)' : 'rgba(232,160,48,0.3)'}`, fontFamily: 'Syne' }}>
            {isAdmin ? '👩‍🏫 Prof' : '🎓 Élève'}
          </div>

          <button onClick={() => setVoiceOpen(!voiceOpen)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all"
            style={{ background: voiceOpen ? 'rgba(232,160,48,0.2)' : 'rgba(86,96,112,0.2)', border: `1px solid ${voiceOpen ? 'rgba(232,160,48,0.5)' : 'rgba(86,96,112,0.3)'}`, color: voiceOpen ? '#fbbf24' : '#8a9ab0', fontFamily: 'Syne' }}>
            <span className={isRecording ? 'recording-pulse inline-block' : ''}>🎤</span>
            <span className="hidden sm:inline">Vocal</span>
            {voices.length > 0 && (
              <span className="w-4 h-4 rounded-full text-xs flex items-center justify-center" style={{ background: '#e8a030', color: '#0f0e0b', fontWeight: 700 }}>
                {voices.length}
              </span>
            )}
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ borderRight: '2px solid rgba(86,96,112,0.25)' }}>
            ...
            {(exercise.pdf_url || exercise.image_url) && (
              <div className="flex gap-1">
                {(['canvas', 'file'] as const).map(v => (
                  <button key={v} onClick={() => setLeftMode(v)} className="text-xs px-2 py-1 rounded transition-all"
          ...
          <div className="flex-1 overflow-hidden">
            {leftMode === 'file' && (exercise.pdf_url || exercise.image_url) ? (
              ...
              <Canvas exerciseId={exerciseId} userId={profile.id} panel="left"
                readonly={!isAdmin} isProf={isAdmin}
                bgImageUrl={exercise.image_url || undefined} onSaved={onSaved} />
            )}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-hidden relative">
            {/* Calque 1 — travail de l'élève (lecture seule pour le prof, modifiable pour l'élève) */}
            ...
            {/* Calque 2 — annotations du prof en rouge (transparent, par-dessus) */}
            {isAdmin && (
        ...
        {voiceOpen && (
          <div className="w-72 flex-shrink-0 flex flex-col"
            ...
            <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
              ...
              <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#8a9ab0', fontFamily: 'Syne' }}>🎤 Messages vocaux</span>
              <button onClick={() => setVoiceOpen(false)} style={{ color: '#566070' }}>✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              {voices.length === 0 && <div className="text-center py-8 text-xs" style={{ color: '#4a5568' }}>Aucun message vocal.</div>}
              {voices.map(msg => {
            ...
            <div className="p-3 flex-shrink-0" style={{ borderTop: '1px solid rgba(86,96,112,0.2)' }}>
              {isRecording ? (
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={stopRecording} className="px-4 py-2 rounded-xl text-xs font-semibold"
                ...
                <button onClick={startRecording} disabled={uploading}
                  ...
                  style={{ background: isAdmin ? 'rgba(48,96,192,0.15)' : 'rgba(232,160,48,0.12)', border: `1px solid ${isAdmin ? 'rgba(48,96,192,0.4)' : 'rgba(232,160,48,0.3)'}`, color: isAdmin ? '#93c5fd' : '#fbbf24', fontFamily: 'Syne' }}>
      ...
      <div className="flex items-center justify-between px-4 py-1.5 flex-shrink-0 text-xs"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span>Cliquez 💾 dans la barre d'outils pour sauvegarder</span>
        </div>
      </div>
    </div>
  )
